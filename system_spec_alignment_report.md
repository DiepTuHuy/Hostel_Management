...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | **Công nghệ Frontend** | Mô tả ứng dụng chạy trên nền tảng **ReactJS** (Web) và **React Native** (Mobile app). | Sử dụng **ReactJS (Vite + Tailwind CSS)** chạy trên trình duyệt. Không có ứng dụng React Native riêng biệt; các cổng di động (Tenant, Visitor) được thiết kế **tương thích hiển thị di động (Responsive Web)**. | **Nhẹ**<br>Hệ thống chạy đúng công nghệ ReactJS cho Web. Phiên bản di động được tối ưu bằng Responsive Web thay vì phát triển app Native riêng. | Cập nhật tài liệu đặc tả: ghi rõ giao diện di động cho Khách thuê và Khách vãng lai được tối ưu hóa qua Responsive Web. |
